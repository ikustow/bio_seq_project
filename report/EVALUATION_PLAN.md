...
На основании 5 белков генерируем 4 вариации, чтобы проверить устойчивость retriever-а:
...
| `V3_full_exact_species` | Полная каноническая последовательность; `context_question` явно называет конкретный вид организма (нужный или исключаемый) | Проверяет, что species-hint в вопросе реально влияет на rerank |
...
2. **Judge LLM — это всё равно LLM.** Текущий judge — Llama 3.3 70B (free через OpenRouter); это **апгрейд** с изначально планировавшейся 8B, после того как OpenRouter выпилил `llama-3.1-8b-instruct:free` из каталога. 70B надёжно судит class A и class B; на class C (многошаговые rubric вида «связывает повторы шафта с протрузией кноба») остаётся остаточный риск шума — судим в одной модели, плюс модель не специализирована под bio-домен. Митигация: атомизировать rubric (одна проверяемая мысль на item), ручной аудит 20% сценариев класса C, плюс честно показывать coverage отдельно по классам, чтобы видеть, где judge ошибается чаще.
