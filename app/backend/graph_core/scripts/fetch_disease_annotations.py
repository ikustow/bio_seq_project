...
def main():
    ...
    disease_rows = []
    batches_dir = outdir / "uniprot_disease_batches"
    ...
    for batch_num, batch in enumerate(batch_iter(accessions, args.batch_size), start=1):
        ...
        if batch_path.exists():
            ...
            disease_rows.extend(frame.to_dict("records"))
            continue
        print(f"Batch {batch_num}: {len(batch)} accessions")
        payload = fetch_batch(batch)
        batch_rows = []
        for entry in payload.get("results", []):
            batch_rows.extend(extract_disease_rows(entry))
        pd.DataFrame(batch_rows).to_parquet(batch_path, index=False)
        disease_rows.extend(batch_rows)
