...
DEFAULT_PCA_DIM = int(os.getenv("BIOSEQ_GRAPH_PCA_DIM", "0"))
ROOT = Path(__file__).resolve().parent.parent
DEFAULT_INPUT = ROOT / "output" / "embeddings.npy"
...
def main():
    ...
    X_norm = normalize(np.asarray(X, dtype=np.float32), norm="l2").astype(np.float32)
