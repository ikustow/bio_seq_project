...
By default Streamlit Cloud looks for `requirements.txt` at repo root. Ours is at `app/frontend/requirements.txt`. Two options — pick one:

**Option A (simplest, recommended):** create a root-level `requirements.txt` that just points at the real one:

```bash
echo "-r app/frontend/requirements.txt" > requirements.txt
git add requirements.txt
git commit -m "Forward root requirements to app/frontend"
git push
```

**Option B:** in the Streamlit Cloud **Advanced settings** of the app, Streamlit Cloud now auto-detects `requirements.txt` next to the main file. If the build uses `app/frontend/requirements.txt` automatically, you're already done and can skip Option A. Check the build log to confirm.
...
- **"ModuleNotFoundError" on first deploy** — you hit Option B but Streamlit didn't pick up `app/frontend/requirements.txt`. Do Option A.
