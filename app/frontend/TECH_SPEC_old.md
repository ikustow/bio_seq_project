...
- A five-turn scripted conversation mirroring [app/frontend/test_chat.txt](test_chat.txt).
...
|---|---|---|---|
| `streamlit` | ≥1.57 | Page framework, `st.chat_*`, `st.columns`, `st.expander`, `st.write_stream` | Gradio — rejected: chat + rich sidebar card is awkward in Blocks |
...
On macOS/Linux the commands differ only in the venv path (`app/frontend/.venv/bin/...`).

## 12. Acceptance criteria (how we know the mock is done)

1. `streamlit run app/frontend/app.py` opens a page with the welcome state and no errors in the console.
